...
//替换img url 中的&
app.filter('replaceImgUrl',function(){
	return function(str){
		if(str!=''){
			return str.replace('@','&');
		}else{
...
//替换img url 中的&
app.filter('replaceNumber',function(){
	return function(str){
...
//Workflow judgment
app.filter('workflowNode',function(){
	return function(arrayDid){
		var node_id;
		 if(arrayDid!=null){
			 for(var i=0;i<arrayDid.length;i++){
				 if(arrayDid[i].wk_cur_sub_node_id==61){
				    node_id=1;
				    break;
				 }else{
					 node_id= 0;
