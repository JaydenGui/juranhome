...
function changess(v){
	...
	var tt=document.getElementById(ss[v]);
...
function changesss(v){
	...
	var aa=document.getElementById(sss[v]);
...
var sss=["sss_province","sss_city","sss_county"];//三个select的name
var opt0 = [];//初始值
...
function _init_areass(){  //初始化函数
	for(var i=0;i<ss.length-1;i++){ 
		try {
			document.getElementById(ss[i]).onchange=new Function("changess("+(i+1)+")");
		} catch (err) { }
	}
	changess(0);
}

function _init_areasss(){  //初始化函数
	for(var i=0;i<sss.length-1;i++){ 
		try {
			document.getElementById(sss[i]).onchange=new Function("changesss("+(i+1)+")");
		} catch (err) { }
	}
	changesss(0);
}