...
function Focus() {
	...
	var iActive = 0;
	function inlize() {
		oPicLis[0].style.filter = 'alpha(opacity:500)';
	...
	byid('tFocus-leftbtnb').onclick = byid('prev').onclick = function() {
		iActive--;
		if (iActive == -1) {
	...
	byid('tFocus-rightbtnb').onclick = byid('next').onclick = function() {
		iActive++;
	...
	byid1('tFocus-leftbtn').onclick = byid('prev').onclick = function() {
		iActive--;
		if (iActive == -1) {
			iActive = oPicLis.length - 1;
		}
	...
	byid1('tFocus-rightbtn').onclick = byid('next').onclick = function() {
		iActive++;
		if (iActive == oPicLis.length) {
			iActive = 0;
		}
...
//Show hidden by pictures bar
$(document).on('click','#asdhsop',function(){
	//$("#tFocusBtn").fadeToggle();});
	if($("#tFocusBtn").hasClass("closed")){
		$("#tFocus-pic").css({height:760},0);
		$("#tFocusBtn").css({height:135},0);
		$("#tFocusBtn").show();
		$("#tFocusBtn").removeClass("closed");
	}else{
		$("#tFocusBtn").css({height:10},0);
		$("#tFocus-pic").css({height:900},0);
		$("#tFocusBtn").hide();
		$("#tFocusBtn").addClass("closed");
	}
});